    ...
    return (
      <div className="flex flex-col items-center justify-center h-[460px]">
        ...
        <Text variant={TextVariants.title} className="mb-3 text-center">
          {t('hdrModal.mergeToHdr')}
        </Text>
        <Text className="text-center max-w-md leading-relaxed">
          {imageCount 
            ? t('hdrModal.description', { count: imageCount })
            : t('hdrModal.descriptionNoCount')
          }
        </Text>
      </div>
    );
  ...
  const renderButtons = () => {
    ...
    if (savedPath) {
      return (
        <>
          ...
          <Button onClick={handleOpen}>{t('hdrModal.openInEditor')}</Button>
        </>
      );
    }
    ...
    return (
      <div className={`w-full flex items-center justify-end gap-2 ${disabled ? 'opacity-50 pointer-events-none' : ''}`}>
        ...
        >
          {finalImageBase64 ? t('hdrModal.close') : t('hdrModal.cancel')}
        </button>
