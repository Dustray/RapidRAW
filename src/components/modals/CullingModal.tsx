...
function ImageThumbnail({ path, thumbnails, isSelected, onToggle, children }: any) {
  ...
  return (
    ...
    >
      <img
        src={thumbnailUrl}
        alt={t('modals.culling.imagePath', { path })}
...
export default function CullingModal({
  ...
  const CULL_ACTIONS: {
    ...
    { value: 'reject', label: t('modals.culling.actions.reject'), icon: <Tag size={16} className="text-red-500" /> },
    { value: 'rate_zero', label: t('modals.culling.actions.rateZero'), icon: <Star size={16} /> },
    { value: 'delete', label: t('modals.culling.actions.delete'), icon: <Trash2 size={16} /> },
  ];
  ...
  const renderSettings = () => (
    <>
      ...
      <div className="space-y-6 text-sm">
        <div>
          <Switch
            label={t('modals.culling.groupSimilarImages')}
          ...
          {settings.groupSimilar && (
            <div className="mt-2 pl-4 border-l-2 border-border-color ml-1">
              ...
              <Text variant={TextVariants.small} className="mt-1">
                {t('modals.culling.similarityDesc')}
              </Text>
            </div>
          )}
        </div>
        <div>
          <Switch
            label={t('modals.culling.filterBlurryImages')}
    ...
    if (totalSuggestions === 0) {
      return (
        <div className="flex flex-col items-center justify-center h-48">
          ...
          <Text>{t('modals.culling.allImagesGood')}</Text>
          <div className="mt-6">
    ...
    return (
      <>
        <Text variant={TextVariants.title} className="mb-4">
          {t('modals.culling.suggestionsTitle')}
        </Text>
        <div className="border-b border-surface mb-4">
          <nav className="-mb-px flex space-x-4" aria-label={t('modals.culling.suggestionsTitle')}>
            {numSimilar > 0 && (
              <button
              ...
              >
                {t('modals.culling.similarGroups')}{' '}
                  ...
                  {suggestions.similarGroups.map((group, index) => (
                    <div key={index} className="bg-surface rounded-lg p-3">
                      <Text variant={TextVariants.heading} className="mb-2">
                        {t('modals.culling.groupLabel', { number: index + 1 })}
                      </Text>
                        ...
                        <div>
                          ...
                          <div className="relative rounded-md overflow-hidden border-2 border-green-500">
                            ...
                            <Text
                              ...
                              className="absolute bottom-0 left-0 right-0 p-1 bg-black/60"
                            >
                              {t('modals.culling.score')}: {group.representative.qualityScore.toFixed(2)}
                            </Text>
                          </div>
                        </div>
                        <div>
                          <Text variant={TextVariants.label} className="mb-1">
                            {t('modals.culling.duplicates')} ({group.duplicates.length})
                          </Text>
                          <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2">
                            {group.duplicates.map((dup) => (
                              ...
                              >
                                {t('modals.culling.score')}: {dup.qualityScore.toFixed(2)}
                              </ImageThumbnail>
                            ))}
                          </div>
                        </div>
              ...
              {activeTab === 'blurry' && (
                <div className="grid grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-2">
                  {suggestions.blurryImages.map((img) => (
                    ...
                    >
                      {t('modals.culling.sharpness')}: {img.sharpnessMetric.toFixed(0)}
                    </ImageThumbnail>
                  ))}
                </div>
              )}
        ...
        <div className="flex justify-between items-center gap-3 mt-6">
          ...
          <div className="flex gap-3">
            ...
            <Button onClick={handleApply} disabled={selectedRejects.size === 0}>
              {t('modals.culling.applyTo', { count: selectedRejects.size, plural: selectedRejects.size !== 1 ? 's' : '' })}
            </Button>
          </div>
        </div>
      </>
